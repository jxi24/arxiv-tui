...
TEST_CASE("Article management", "[database]") {
    ...
    SECTION("Should add and retrieve articles") {
        ...
        REQUIRE_CALL(db, GetRecent(-1)).RETURN(std::vector<Arxiv::Article>{article});
    ...
    SECTION("Should handle bookmarks") {
        ...
        REQUIRE_CALL(db, ListBookmarked()).RETURN(std::vector<Arxiv::Article>{article});
        ...
        REQUIRE_CALL(db, ListBookmarked()).RETURN(std::vector<Arxiv::Article>{});
...
TEST_CASE("Project management", "[database]") {
    ...
    SECTION("Should manage projects") {
        ...
        REQUIRE_CALL(db, GetProjects()).RETURN(std::vector<std::string>{project_name});
        ...
        REQUIRE_CALL(db, GetProjects()).RETURN(std::vector<std::string>{});
    ...
    SECTION("Should link articles to projects") {
        ...
        REQUIRE_CALL(db, GetArticlesForProject(project_name)).RETURN(std::vector<Arxiv::Article>{});
