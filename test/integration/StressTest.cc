...
static CoreBundle make_core(const std::vector<Arxiv::Article>& articles) {
    ...
    b.tmp_dir = fs::temp_directory_path() / ("arxiv_stress_test_" + std::to_string(::getpid()));
...
static std::string jline(const std::string& json_body) { return json_body + "\n"; }
...
TEST_CASE("Stress: browsing and bookmarking session", "[integration][stress]") {
    ...
    script += jline(R"({"ts":4000,"action":"toggle_bookmark","article_link":")" + articles[5].link +
                    R"("})");
...
TEST_CASE("Stress: full project lifecycle", "[integration][stress]") {
    ...
    script += jline(
        R"({"ts":1003,"action":"set_project_parent","project":"Quantum","parent":"Physics"})");
    script += jline(
        R"({"ts":1004,"action":"set_project_parent","project":"Lattice","parent":"Physics"})");
    ...
    script +=
        jline(R"({"ts":3000,"action":"set_project_note","project":"Quantum","article_link":")" +
              articles[0].link + R"(","note":"Key result on entanglement"})");
    script +=
        jline(R"({"ts":3001,"action":"set_project_note","project":"Lattice","article_link":")" +
    ...
    script +=
        jline(R"({"ts":5000,"action":"export_project_markdown","project":"Quantum","path":")" +
              md_path + R"("})");
    script += jline(R"({"ts":5001,"action":"export_project_text","project":"Quantum","path":")" +
                    txt_path + R"("})");
    script += jline(R"({"ts":5002,"action":"export_project_json","project":"Quantum","path":")" +
                    json_path + R"("})");
    script += jline(R"({"ts":5003,"action":"export_project_bibtex","project":"Quantum","path":")" +
                    bib_path + R"("})");
...
TEST_CASE("Stress: rating and ranking workflow", "[integration][stress]") {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        script += jline(R"({"ts":)" + std::to_string(1000 + i) +
                        ...
                        articles[static_cast<std::size_t>(i)].link + R"(","rating":)" +
...
TEST_CASE("Stress: search and date-range filtering", "[integration][stress]") {
    ...
    script +=
        jline(R"({"ts":1000,"action":"set_date_range","start":"2024-01-01","end":"2026-12-31"})");
    ...
    script += jline(
        R"({"ts":2000,"action":"set_search_query","query":"quantum","title":true,"authors":false,"abstract_field":true})");
    ...
    script += jline(
        R"({"ts":3000,"action":"set_search_query","query":"Stress","title":true,"authors":false,"abstract_field":false})");
    script += jline(R"({"ts":3001,"action":"set_filter_index","index":4})");

    // Search with empty query (edge case)
    script += jline(
        R"({"ts":4000,"action":"set_search_query","query":"","title":true,"authors":true,"abstract_field":true})");
...
TEST_CASE("Stress: rapid filter and index cycling", "[integration][stress]") {
    ...
    for (int round = 0; round < 50; ++round) {
        for (int filter = 0; filter <= 6; ++filter) {
            script +=
                jline(R"({"ts":)" + std::to_string(ts++) +
                      R"(,"action":"set_filter_index","index":)" + std::to_string(filter) + "}");
        }
    }
...
TEST_CASE("Stress: export project as JSON then import into fresh core", "[integration][stress]") {
    ...
    setup +=
        jline(R"({"ts":2000,"action":"set_project_note","project":"RoundTrip","article_link":")" +
    ...
    import_script +=
        jline(R"({"ts":4000,"action":"import_project_json","path":")" + json_path + R"("})");
...
TEST_CASE("Stress: BibTeX export for single and project articles", "[integration][stress]") {
    ...
    for (int i = 0; i < 3; ++i) {
        script +=
            jline(R"({"ts":)" + std::to_string(2001 + i) +
                  R"(,"action":"link_article_to_project","article_link":")" +
                  articles[static_cast<std::size_t>(i)].link + R"(","project":"BibProject"})");
    }
    script +=
        jline(R"({"ts":3000,"action":"export_project_bibtex","project":"BibProject","path":")" +
              project_bib + R"("})");
...
TEST_CASE("Stress: edge cases do not crash", "[integration][stress]") {
    ...
    script += jline(
        R"({"ts":1000,"action":"toggle_bookmark","article_link":"https://arxiv.org/abs/9999.99999"})");
    ...
    script += jline(
        R"({"ts":1004,"action":"set_project_parent","project":"Ghost","parent":"AlsoGhost"})");
    ...
    script += jline(R"({"ts":1006,"action":"rate_article","article_link":")" + articles[0].link +
                    R"(","rating":0})");
    script += jline(R"({"ts":1007,"action":"rate_article","article_link":")" + articles[0].link +
                    R"(","rating":100})");
    ...
    script += jline(
        R"({"ts":1011,"action":"export_project_markdown","project":"NoSuchProject","path":")" +
        dead_path + R"("})");
    script +=
        jline(R"({"ts":1012,"action":"export_project_bibtex","project":"NoSuchProject","path":")" +
              dead_path + R"("})");

    // Import from non-existent file
    script +=
        jline(R"({"ts":1013,"action":"import_project_json","path":"/tmp/no_such_file_xyz.json"})");

    // Search with special characters
    script += jline(
        R"({"ts":1014,"action":"set_search_query","query":"O'Brien \"quoted\" (parens) [brackets]","title":true,"authors":true,"abstract_field":true})");
...
TEST_CASE("Stress: full realistic session combining all features", "[integration][stress]") {
    ...
    script += jline(R"({"ts":)" + std::to_string(ts++) +
                    R"(,"action":"set_filter_index","index":1})"); // Bookmarks
    script += jline(R"({"ts":)" + std::to_string(ts++) +
                    R"(,"action":"set_filter_index","index":0})"); // All

    // --- Phase 2: Create projects and organize ---
    script += jline(R"({"ts":)" + std::to_string(ts++) +
                    R"(,"action":"add_project","name":"Session-Root"})");
    script += jline(R"({"ts":)" + std::to_string(ts++) +
                    R"(,"action":"add_project","name":"Session-Child"})");
    script += jline(
    ...
    script +=
        jline(R"({"ts":)" + std::to_string(ts++) + R"(,"action":"set_filter_index","index":4})");
    script += jline(R"({"ts":)" + std::to_string(ts++) +
                    R"(,"action":"set_date_range","start":"2024-01-01","end":"2026-12-31"})");
    script +=
        jline(R"({"ts":)" + std::to_string(ts++) + R"(,"action":"set_filter_index","index":3})");
    ...
    script += jline(R"({"ts":)" + std::to_string(ts++) +
                    R"(,"action":"remove_project","name":"Session-Child"})");
    script +=
        jline(R"({"ts":)" + std::to_string(ts++) + R"(,"action":"set_filter_index","index":0})");

    // --- Phase 7: Recommended filter ---
    script +=
        jline(R"({"ts":)" + std::to_string(ts++) + R"(,"action":"set_filter_index","index":5})");
    script +=
        jline(R"({"ts":)" + std::to_string(ts++) + R"(,"action":"set_article_index","index":0})");
    script +=
        jline(R"({"ts":)" + std::to_string(ts++) + R"(,"action":"set_filter_index","index":0})");
