...
inline int Similarity(const std::string& a, const std::string& b) {
    ...
    std::transform(a.begin(), a.end(), la.begin(), [](unsigned char c) {
        return static_cast<char>(std::tolower(c));
    });
    std::transform(b.begin(), b.end(), lb.begin(), [](unsigned char c) {
        return static_cast<char>(std::tolower(c));
    });
...
inline bool MatchesText(const std::string& query, const std::string& text, int threshold) {
    ...
    std::transform(query.begin(), query.end(), lq.begin(), [](unsigned char c) {
        return static_cast<char>(std::tolower(c));
    });
    std::transform(text.begin(), text.end(), lt.begin(), [](unsigned char c) {
        return static_cast<char>(std::tolower(c));
    });

    // Whole-string similarity
    if (Similarity(lq, lt) >= threshold)
        return true;
    ...
    for (std::size_t i = 0; i < steps; ++i) {
        ...
        if (Similarity(lq, sub) >= threshold)
            return true;
    }
    return false;
}
