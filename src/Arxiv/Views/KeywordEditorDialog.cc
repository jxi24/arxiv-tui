...
void ArxivApp::SetupKeywordEditorDialog() {
    keyword_dialog = Renderer([&] {
        ...
        if (keyword_edit_list.empty()) {
            kw_entries.push_back(text("  (no keywords — add one below)") |
            ...
            for (int i = 0; i < static_cast<int>(keyword_edit_list.size()); ++i) {
                ...
                if (selected)
                    kw_entries.push_back(entry | bold | color(TextColors::base()) |
        ...
        return vbox(std::move(kw_entries)) | borderStyled(ROUNDED, TextColors::border()) |
...
bool ArxivApp::HandleKeywordEditorEvent(ftxui::Event event) {
    ...
    if (event == Event::Delete || event == Event::Character("d")) {
        if (!keyword_edit_list.empty()) {
            keyword_edit_list.erase(keyword_edit_list.begin() + keyword_selected_index);
            keyword_selected_index = std::max(
                0,
                std::min(keyword_selected_index, static_cast<int>(keyword_edit_list.size()) - 1));
        }
        return true;
    }
    if (key_bindings.matches(event, KeyBindings::Action::Next)) {
        keyword_selected_index =
            std::min(keyword_selected_index + 1,
    ...
    if (event == Event::Backspace) {
        if (!keyword_new_entry.empty())
            keyword_new_entry.pop_back();
        return true;
    }
    if (event == Event::Escape) {
        if (m_recorder)
            m_recorder->RecordSaveKeywords(keyword_edit_list);
