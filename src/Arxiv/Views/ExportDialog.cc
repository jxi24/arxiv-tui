...
void ArxivApp::SetupExportDialog() {
    export_dialog = Renderer([&] {
        ...
        static const char* formats[] = {
            "Markdown (.md)", "Plain Text (.txt)", "JSON (.json)", "BibTeX (.bib)"};
        Elements items;
        for (int i = 0; i < 4; ++i) {
            auto item = text(std::string(i == export_format_index ? "> " : "  ") + formats[i]);
            items.push_back(i == export_format_index ? item | bold | color(TextColors::primary())
        ...
        return vbox({
                   text("Export Project: " + export_project_name) | bold |
                   ...
                   separator() | color(TextColors::border()),
                   text("j/k to select format, Enter to export, Esc to cancel") |
...
bool ArxivApp::HandleExportEvent(ftxui::Event event) {
    if (event == Event::Return) {
        ...
        if (export_format_index == 0) {
            ...
            if (m_recorder)
                m_recorder->RecordExportProjectMarkdown(export_project_name, path);
        } else if (export_format_index == 1) {
            ok = core.ExportProjectText(export_project_name, path);
            if (m_recorder)
                m_recorder->RecordExportProjectText(export_project_name, path);
        } else if (export_format_index == 2) {
            ok = core.ExportProjectJSON(export_project_name, path);
            if (m_recorder)
                m_recorder->RecordExportProjectJSON(export_project_name, path);
        } else {
            ok = core.ExportProjectBibTeX(export_project_name, path);
            if (m_recorder)
                m_recorder->RecordExportProjectBibTeX(export_project_name, path);
        }
        if (ok) {
