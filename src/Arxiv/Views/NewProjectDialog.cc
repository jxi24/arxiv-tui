...
ftxui::Element ArxivApp::RenderNewProjectDialog() {
    ...
    if (!parent_for_new_project.empty()) {
        new_proj_elements.push_back(text("Parent: " + parent_for_new_project) |
                                    color(TextColors::subtext()));
    }
    new_proj_elements.push_back(separator() | color(TextColors::border()));
    new_proj_elements.push_back(text("Press Enter to create, Esc to cancel") |
                                color(TextColors::subtext()) | center);

    return vbox(new_proj_elements) | borderStyled(ROUNDED, TextColors::border()) |
...
bool ArxivApp::HandleNewProjectEvent(ftxui::Event event) {
    if (event == Event::Return) {
        if (!new_project_name.empty()) {
            ...
            if (m_recorder)
                m_recorder->RecordAddProject(new_project_name);
            if (!parent_for_new_project.empty()) {
                core.SetProjectParent(new_project_name, parent_for_new_project);
                if (m_recorder)
                    m_recorder->RecordSetProjectParent(new_project_name, parent_for_new_project);
            }
        }
